const List<String> empltyList = [];
const complexitys = ['简单', '中等', '复杂'];

class MealModel {
  String id = '';
  List<String> categories = [];
  String title = '';
  int affordability = 0;
  int complexity = 0;
  String complexityStr = '';
  String imageUrl = '';
  int duration = 0;
  List<String> ingredients = [];
  List<String> steps = [];
  bool isGlutenFree = false;
  bool isVegan = false;
  bool isVegetarian = false;
  bool isLactoseFree = false;

  MealModel({
    required this.id,
    this.categories = empltyList,
    this.title = '',
    this.affordability = 0,
    this.complexity = 0,
    this.imageUrl = '',
    this.duration = 0,
    this.ingredients = empltyList,
    this.steps = empltyList,
    this.isGlutenFree = false,
    this.isVegan = false,
    this.isVegetarian = false,
    this.isLactoseFree = false,
  });

  MealModel.fromJson(Map<String, dynamic> json) {
    id = json['id'];
    categories = json['categories'].cast<String>();
    title = json['title'];
    affordability = json['affordability'];
    complexity = json['complexity'];
    complexityStr = complexitys[complexity];
    imageUrl = json['imageUrl'];
    duration = json['duration'];
    ingredients = json['ingredients'].cast<String>();
    steps = json['steps'].cast<String>();
    isGlutenFree = json['isGlutenFree'];
    isVegan = json['isVegan'];
    isVegetarian = json['isVegetarian'];
    isLactoseFree = json['isLactoseFree'];
  }

  Map<String, dynamic> toJson() {
    final Map<String, dynamic> data = new Map<String, dynamic>();
    data['id'] = this.id;
    data['categories'] = this.categories;
    data['title'] = this.title;
    data['affordability'] = this.affordability;
    data['complexity'] = this.complexity;
    data['imageUrl'] = this.imageUrl;
    data['duration'] = this.duration;
    data['ingredients'] = this.ingredients;
    data['steps'] = this.steps;
    data['isGlutenFree'] = this.isGlutenFree;
    data['isVegan'] = this.isVegan;
    data['isVegetarian'] = this.isVegetarian;
    data['isLactoseFree'] = this.isLactoseFree;
    return data;
  }

  @override
  bool operator ==(other) {
    // 判断是否是非
    if (other is! MealModel) {
      return false;
    }
    final MealModel meal = other;
    return id == meal.id;
  }

  @override
  int get hashCode => super.hashCode;
}
